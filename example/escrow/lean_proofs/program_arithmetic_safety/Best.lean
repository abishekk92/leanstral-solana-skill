...
def initializeTransition (p_preState : ProgramState) (p_amount p_taker_amount : Nat) : Option ProgramState :=
  if p_amount <= U64_MAX ∧ p_taker_amount <= U64_MAX then
    some { p_preState with trackedTotal := p_preState.trackedTotal + p_amount + p_taker_amount }
  else
    none
